...
with section_defaults(id, type, title, subtitle, config, position) as (
  values
    (
      ...
      'deals'::text,
      'Today''s Best Deals'::text,
      'Save on selected products'::text,
      '{"limit":4,"cta_label":"View all products","cta_url":"/product"}'::jsonb,
      1
    ),
    (
      '60000000-0000-4000-8000-000000000014'::uuid,
      'new_arrivals'::text,
      'New Arrival Products'::text,
      'Discover the latest additions'::text,
      '{"limit":4,"cta_label":"View all products","cta_url":"/product"}'::jsonb,
      2
    )
), sort_base as (
