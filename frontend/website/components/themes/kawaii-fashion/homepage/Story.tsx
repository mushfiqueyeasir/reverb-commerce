...
const FALLBACK_SERVICES = [
  {
    id: "edit",
    icon: Sparkles,
    label: "Considered edit",
    detail: "Chosen with care",
  },
  { id: "fit", icon: Shirt, label: "Everyday ease", detail: "Made to be worn" },
  {
    id: "finish",
    icon: Award,
    label: "Lovely details",
...
export default function KawaiiFashionStory({
  ...
  const services = story.cards.length
  ...
  return (
    <section className="relative overflow-hidden bg-background py-16 sm:py-24 lg:py-32">
      <div className="pointer-events-none absolute left-0 top-1/3 h-64 w-64 -translate-x-1/2 rounded-full bg-primary/10 blur-3xl" />
      <div className="relative mx-auto grid max-w-[1500px] gap-10 px-5 sm:px-6 lg:grid-cols-2 lg:items-center lg:gap-16 lg:px-10 xl:gap-24">
        <figure className="relative mx-auto w-full max-w-2xl lg:mx-0">
          <div className="relative aspect-[4/5] overflow-hidden border border-border bg-surface sm:aspect-[5/6]">
            {imageUrl ? (
              <Image
                src={imageUrl}
                alt={story.imageAlt || title || "Our fashion story"}
                fill
        ...
        <div className="pt-5 lg:pt-0">
          <p className="mb-4 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.24em] text-primary">
            ...
            {eyebrow || story.copyLabel || "More than a wardrobe"}
          </p>
          {title ? (
